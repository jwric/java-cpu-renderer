...
public class StrokeShader extends Shader {
    ...
    public Vec4 strokeColor = new Vec4(0,0,0,1f);
    public float strokeSize = 0.0625f/3f;
    public static Texture frontTexture = AWTARGBTexture.createFromFile("front.png");
    public Texture sampler = frontTexture;
    ...
    public void fragment(Vec4 gl_FragCoord, final VertexAttributes out, final Vec4 gl_FragColor) {
        ...
        {
            ColorUtils.vec4(baseColor, gl_FragColor);
        }

        if (uv.x > 1 - strokeSize || uv.x < strokeSize || uv.y > 1 - strokeSize || uv.y < strokeSize) {
            gl_FragColor.set(strokeColor);
        }
